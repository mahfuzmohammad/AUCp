...
from dataloaders.dataload import MedAD, BraTSAD, OCT2017, ColonAD, ISIC2018, CpChildA, Camelyon16AD, MedAD_aucp, BraTSAD_aucp, ISIC2018_aucp, Camelyon16AD_aucp
import wandb
...
class BaseWorker:
    ...
    def set_test_loader_aucp(self):
        """ Use for only AUCp evaluation"""
        data_path = get_data_path(dataset=self.opt.dataset)
        test_transform = get_transform(self.opt)
        if self.opt.dataset in ['rsna', 'vin', 'brain', 'lag']:
            self.test_set = MedAD_aucp(main_path=data_path, img_size=self.opt.model['input_size'], transform=test_transform,
                                  mode='test')
        elif self.opt.dataset == 'brats':
            self.test_set = BraTSAD_aucp(main_path=data_path, img_size=self.opt.model['input_size'],
                                    transform=test_transform, mode='test')
        elif self.opt.dataset == 'c16':
            self.test_set = Camelyon16AD_aucp(main_path=data_path, img_size=self.opt.model['input_size'],
                                         transform=test_transform, mode='test', n_channel=self.opt.model["in_c"])
        elif self.opt.dataset == "isic":
            self.test_set = ISIC2018_aucp(main_path=data_path, img_size=self.opt.model['input_size'],
                                     transform=test_transform, mode='test')
        else:
            raise Exception("Invalid dataset: {}".format(self.opt.dataset))
        self.test_loader = DataLoader(self.test_set, batch_size=1, shuffle=False)
    ...
    def load_checkpoint(self, epoch=None):
        ...
        model_path = os.path.join(self.opt.train['save_dir'], "checkpoints", f"epoch_{epoch}.pt")
        self.net.load_state_dict(torch.load(model_path, map_location=torch.device("cuda:{}".format(self.opt.gpu))))
        print("=> Load model from {}".format(model_path))

    def run_eval(self, epoch=None, aucp=False):
        results = self.evaluate()
        # metrics_save_path = os.path.join(self.opt.train['save_dir'], "metrics.txt")
        if aucp:
            metrics_save_path = os.path.join(self.opt.train['save_dir'], f"aucp_metrics_epoch_{epoch}.txt")
            with open(metrics_save_path, "w") as f:
                for key, value in results.items():
                    f.write(str(key) + ": " + str(value) + "\n")
                    print(key + ": {:.4f}".format(value))
        else:
            metrics_save_path = os.path.join(self.opt.train['save_dir'], f"auc_metrics_epoch_{epoch}.txt")
            with open(metrics_save_path, "w") as f:
                for key, value in results.items():
                    f.write(str(key) + ": " + str(value) + "\n")
                    print(key + ": {:.4f}".format(value))
