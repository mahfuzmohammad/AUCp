...
class Camelyon16AD_aucp(Dataset):
    def __init__(self, main_path, size=64, transform=None, mode="train"):
...
class ISIC2018_aucp(Dataset):
    def __init__(self, main_path, size=64, transform=None, mode="train"):
        ...
        if mode == 'train':
            ...
            t0 = time.time()
            self.slices += parallel_load(test_data_dir, test_normal, size)
            self.slices += parallel_load(test_data_dir, test_abnormal, size)
            self.labels += len(test_normal) * [1] + len(test_abnormal) * [1]
            self.img_ids += test_normal + test_abnormal

            self.slices += parallel_load(train_data_dir, train_normal, size)
            self.labels += [0] * len(train_normal)
            self.img_ids += train_normal
