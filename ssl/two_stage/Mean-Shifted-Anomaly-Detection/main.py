...
def train_model(model, train_loader, test_loader, train_loader_1, device, args):
    wandb.init(project="Mean-Shifted-AD", config=vars(args))
    # wandb.init(project="tmp", config=vars(args))
    # example_in = torch.zeros((1, 3, 224, 224)).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        gde_auc, gde_ap, knn_auc, knn_ap, _ = get_score(model, device, train_loader, test_loader)
        print('Epoch: {}, GDE AUROC is: {}, AP is: {}; KNN AUROC is: {}, AP is: {}'.format(
