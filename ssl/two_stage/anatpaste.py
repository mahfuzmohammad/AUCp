...
class AnatPaste(CutPaste):
    ...
    def __call__(self, img):
        ...
        if lung_area.sum() > 0:
            ...
            coord = [np.random.randint(0, w), np.random.randint(0, h)]  # Fixed: use actual image dimensions
            lung_area = np.ones((h, w))  # Fixed: create proper mask with image dimensions
        ...
        mask = Image.new("L", (w, h))  # Fixed: corrected width/height order
        draw = ImageDraw.Draw(mask)

        # Ensure margin doesn't make dimensions invalid
        effective_margin = min(self.margin, cut_w // 4, cut_h // 4)
        
        # Calculate rectangle bounds with proper boundary checks
        x1 = max(0, coord[0] - cut_w // 2 + effective_margin)
        y1 = max(0, coord[1] - cut_h // 2 + effective_margin)
        x2 = min(w, coord[0] + cut_w // 2 - effective_margin)
        y2 = min(h, coord[1] + cut_h // 2 - effective_margin)
        ...
        if np.random.random() < 0.5:
            draw.ellipse((x1, y1, x2, y2), fill=mask_value)
        else:
        ...
        # Convert mask to numpy array for intersection calculation
        mask_array = np.array(mask_blur)
        intersection = mask_array * lung_area.astype(np.uint8)
        intersection = Image.fromarray(intersection.astype(np.uint8))

        patch_im = Image.new("L", (w, h))  # Fixed: corrected width/height order
        
        # Calculate paste position with boundary checks
        paste_x = max(0, min(w - cut_w, coord[0] - cut_w // 2))
        paste_y = max(0, min(h - cut_h, coord[1] - cut_h // 2))
        patch_im.paste(patch, (paste_x, paste_y))
