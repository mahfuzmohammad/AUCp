...
def get_train_embeds(model, dataloader_normal, device):
    train_embed = []
    with torch.no_grad():
        for batch in dataloader_normal:
...
if __name__ == '__main__':
    ...
    model_paths = os.listdir(out_dir)
    model_paths = sorted(
        [os.path.join(out_dir, p) for p in os.listdir(out_dir) if re.search(r'_epoch_(\d+)\.tch$', p)],
        key=lambda x: int(re.search(r'_epoch_(\d+)', x).group(1))
    )
    ...
    if data_type in ['rsna', 'vin', 'brain', 'lag']:
        ...
        test_data_eval_aucp = MedAD_aucp(data_path, transform=test_transform, size=size, mode="test")
        normal_data_aucp = MedAD_aucp(data_path, transform=test_transform, size=size, mode="train")
        ...
        test_data_eval_aucp = BraTSAD_aucp(data_path, transform=test_transform, size=size, mode="test")
        normal_data_aucp = BraTSAD_aucp(data_path, transform=test_transform, size=size, mode="train")
        ...
        test_data_eval_aucp = Camelyon16AD_aucp(data_path, transform=test_transform, size=size, mode="test")
        normal_data_aucp = Camelyon16AD_aucp(data_path, transform=test_transform, size=size, mode="train")
        ...
        test_data_eval_aucp = ISIC2018_aucp(data_path, transform=test_transform, size=size, mode="test")
        normal_data_aucp = ISIC2018_aucp(data_path, transform=test_transform, size=size, mode="train")
    ...
    dataloader_test_aucp = DataLoader(test_data_eval_aucp, batch_size=1, shuffle=False, num_workers=0)
    dataloader_normal_aucp = DataLoader(normal_data_aucp, batch_size=64, shuffle=False, num_workers=0)
    ...
    epoch = 0
    if os.path.exists(os.path.join(out_dir, "aucp_results.csv")):
        os.remove(os.path.join(out_dir, "aucp_results.csv"))
    for model_name in model_paths:
        print(f"evaluating model {model_name.split('_')[-1].split('.')[0]}")
        ...
        test_roc_aucp, test_ap, cls_auc, cls_ap = eval_model(model_name, device=device,
                                                            save_plots=False,
                                                            size=size,
                                                            show_training_data=False,
                                                            # model=model,
                                                            mode='valid',
                                                            dataloader_test=dataloader_test_aucp,
                                                            dataloader_normal=dataloader_normal_aucp)
        results_aucp = {"AUC": test_roc_aucp, "AP": test_ap, "cls_auc": cls_auc, "cls_ap": cls_ap}
        for key, value in results_aucp.items():
            # f.write(str(key) + ": " + str(value) + "\n")
            print(key + " (AUCp): {}".format(value))
        
        with open(os.path.join(out_dir, "aucp_results.csv"), "a") as f:
            csvwriter = csv.writer(f)
            csvwriter.writerow(['epoch', 'model_path', 'AUC', 'AUCp', 'AP'])
            csvwriter.writerow([epoch, model_name, test_roc_auc, test_roc_aucp, test_ap])
        epoch += 1
